...
import EditDorm from "../components/Popup/EditDorm";
import EditBuilding from "../components/Popup/EditBuilding";
...
export default function Management() {
  const [dormitoryDefaultData, setDormitoryDefaultData] = useState<
    dormitoryInfoInterface[]
  >([]);
  ...
  const [valueSelectedDormitory, setValueSelectedDormitory] =
    useState<number>(-1);
  ...
  const getBuildData = async (_dorm: dormitoryInterface[]) => {
    try {
      const buildingPromises =
        _dorm &&
        _dorm.map(async (dormitory) => {
          const res = await apiClient(
            `${API}/Building/GetAllBuilding/${dormitory.idDormitory}`,
            {
              method: "GET",
            }
          );
      ...
      const flattenedBuildingData: buildingInterface[] =
        buildingResults.flatMap(
          (buildingArray: any[]) => buildingArray
  ...
  const getRoomStatusData = async (_building: buildingInterface[]) => {
    try {
      const buildingAndRoomPromises =
        _building &&
        _building.map(async (building) => {
          const res = await apiClient(
            `${API}/Room/GetAllRoomStatus/${building.idBuilding}`,
            {
              method: "GET",
            }
          );
  ...
  const mergeData = (
    dorm: dormitoryInterface[],
    buildingAndRoomResults: buildingInfoInterface[]
  ) => {
    const updatedDormitoryData = dorm.map((dormitory) => {
      // ค้นหา building ที่มี idDormitory ตรงกับ idDormitory ของ dormitory ปัจจุบัน
      const matchingBuilding = buildingAndRoomResults.filter(
        (building) => building.idDormitory === dormitory.idDormitory
      );
      if (matchingBuilding) {
  ...
  const { status: isEditDormOpen, toggleStatus: setIsEditDormOpen } =
    useToggle();

  const { status: isEditBuildOpen, toggleStatus: setIsEditBuildOpen } =
    useToggle();
  ...
  const handleOpenDelBuildDialog = () =>
    setOpenDelBuildDialog(!openDelBuildDialog);
  ...
  const [openAccordionDromIndexes, setOpenAccordionDromIndexes] = useState<
    boolean[]
  >([]);
  ...
  const [openAccordionBuildIndexes, setOpenAccordionBuildIndexes] = useState<
    boolean[]
  >([]);
  ...
  const setNewDataSelected = (val: number) => {
    ...
    const selectedBuilding =
      dormitoryDefaultData[valueSelectedDormitory].buildingInfo[val];
      ...
      {dormitoryData.length != 0 ? (
        <div>
          <div className="flex my-4 items-center justify-between">
              ...
              <Popover placement="bottom-end">
                ...
                <PopoverContent className="flex flex-col gap-2 ">
                  <Select
                    ...
                    onChange={(val) => {
                      setValueSelectedDormitory(parseInt(val));
                      setDormitoryData([dormitoryDefaultData[parseInt(val)]]);
                    }}
                  >
                  ...
                  >
                    {dormitoryDefaultData &&
                    dormitoryDefaultData[valueSelectedDormitory] &&
                    dormitoryDefaultData[valueSelectedDormitory]
                      .buildingInfo ? (
                      dormitoryDefaultData[
                        valueSelectedDormitory
                      ].buildingInfo.map((building, buildingIndex) => (
                        <Option
                          key={building.idBuilding}
                          value={buildingIndex.toString()}
                        >
            ...
            <div className="hidden md:flex gap-5">
              ...
              >
                {dormitoryDefaultData &&
                dormitoryDefaultData[valueSelectedDormitory] &&
                dormitoryDefaultData[valueSelectedDormitory].buildingInfo ? (
                  dormitoryDefaultData[valueSelectedDormitory].buildingInfo.map(
          ...
          <div className="flex justify-between">
            <div className="w-full xl:w-[80%]">
              {dormitoryData &&
                dormitoryData.map((dormData, dormIndex) => (
                  <Card className="px-5 py-1 mb-5 xl:mr-5 h-fit overflow-auto min-w-[500px]">
                      ...
                      <AccordionHeader className="font-Montserrat text-base border-b-0 cursor-default">
                        <div className="flex justify-between w-full mr-[-16px]">
                          ...
                          <div className="flex gap-4">
                            ...
                            <DeletePopup
                              open={openDelDormDialog}
                              handleDialog={handleDelDormDialog}
                            /> */}
                            <button
                              onClick={() =>
                                handleToggleAccordionDorm(dormIndex)
                              }
                            >
                              <Icon
                                id={dormIndex}
                                open={openAccordionDromIndexes[dormIndex]}
                              />
                            </button>
                          </div>
                        </div>
                                ...
                                >
                                  ...
                                  >
                                    <div className="flex justify-between w-full mr-[-16px]">
                                      ...
                                      <div className="flex gap-4">
                                        <AddRoom
                                          data={buildingData.idBuilding}
                                        />
                                        <Cog6ToothIcon
                                          width={26}
                                          onClick={setIsEditBuildOpen}
                                          className="cursor-pointer"
                                        />
                                        <EditBuilding
                                  ...
                                  <AccordionBody className="grid grid-cols-5 lg:grid-cols-8 xl:grid-cols-7 2xl:grid-cols-10">
                                    {buildingData &&
                                        ...
                                        <div>
                                          {roomData.isRoomStay ? (
                                            <Card className="flex m-1 h-14 rounded-md justify-center items-center border min-w-[85px] shadow-none bg-prim hover:bg-prim2 ">
                                              ...
                                              >
                                                {roomData.isRoomLatePay ? (
                                                  <ExclamationCircleIcon
                                                    color="#AE2012"
                                                    className="absolute top-[-5px] right-[-5px] w-4 h-4"
                                                  />
                                                ) : roomData.isRoomPay ? (
                                                  <CheckCircleIcon className="absolute top-[-5px] right-[-5px] w-4 h-4 text-success" />
                                                ) : (
                                                  <ClockIcon className="absolute top-[-5px] right-[-5px] w-4 h-4 text-clock " />
                                                )}
                                                <span className="font-bold text-base text-a">
                                                  {roomData.roomName}
                                                </span>
                                              </button>
                                              <button
                                            ...
                                            <Card className="flex m-1 h-14 rounded-md justify-center items-center border min-w-[85px] shadow-none hover:bg-[#fff595] focus:outline-none focus:ring focus:ring-red-300">
                                              ...
                                              >
                                                <ClockIcon
                                                  color="#ECB92F"
                                                  className="absolute top-[-5px] right-[-5px] w-4 h-4 text-clock"
                                                />
                                                <span className="font-bold text-base ">
                                                  {roomData.roomName}
                                                </span>
              ...
              <DialogBody>
                ...
                <Tabs value="detail">
                  <TabsHeader className="flex items-center mt-5">
                    {tabsData.map(({ label, value }) => (
                      <Tab key={value} value={value} className="text-lg py-2">
                        {label}
                      </Tab>
                    ))}
                  </TabsHeader>
                  <TabsBody>
                    ...
                    <TabPanel key={tabsData[1].value} value={tabsData[1].value}>
                      <Lease data={idRoom} />
                    </TabPanel>
                    <TabPanel key={tabsData[2].value} value={tabsData[2].value}>
                      <PaymentHistory data={idRoom} />
                    </TabPanel>
                    <TabPanel key={tabsData[3].value} value={tabsData[3].value}>
                      <Report data={idRoom} />
                    </TabPanel>
                  </TabsBody>
                </Tabs>
        ...
        <div className="flex flex-col items-center justify-center h-[80vh]">
          <Typography variant="h5" className="mb-5">
            You don't have a dormitory
          </Typography>
          <Button className="flex items-center gap-2 " onClick={setIsOpen}>
          ...
          <AddDorm open={isOpen} handler={setIsOpen} />
        </div>
      )}
    </div>
  );
